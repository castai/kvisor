...
func BenchmarkAA(b *testing.B) {
	kout := make([]TrafficKey, 1000)
	vout := make([]TrafficSummary, 1000)
	...
	b.Run("slice", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			resultKeys := make([]TrafficKey, 0, 1000)
			resultVals := make([]TrafficSummary, 0, 1000)
			for ii := range kout {
				resultKeys = append(resultKeys, kout[ii])
				resultVals = append(resultVals, vout[ii])
			}
		}
	})
}

func fillKey(kout []TrafficKey) {
	for i := range kout {
		kout[i] = TrafficKey{
			...
			Tuple: struct {
				...
				Saddr: struct{ Raw [16]uint8 }{
					Raw: [16]uint8{0x01, 0x02, 0x03, 0x04},
				},
				Daddr: struct{ Raw [16]uint8 }{
					Raw: [16]uint8{0x01, 0x02, 0x03, 0x04},
				},
...
func fillSumm(sum []TrafficSummary) {
	for i := range sum {
		sum[i] = TrafficSummary{
			RxPackets:    1,
			RxBytes:      2,
			TxPackets:    3,
			TxBytes:      4,

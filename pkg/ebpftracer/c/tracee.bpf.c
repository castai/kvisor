...
statfunc int send_socket_connect(struct pt_regs *ctx, program_data_t *p)
{
...
statfunc int send_tty_write(struct pt_regs *ctx, program_data_t *p, struct file *file) {
...
do_file_io_operation(struct pt_regs *ctx, u32 event_id, u32 tail_call_id, bool is_read, bool is_buf)
{
    ...
    if (should_submit(TTY_WRITE, p.event)) {
        send_tty_write(ctx, &p, (struct file *) saved_args.args[0]);
        reset_event_args(&p);
    }

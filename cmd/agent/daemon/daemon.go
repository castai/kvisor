...
func NewRunCommand(version string) *cobra.Command {
	...
	var (
		...
		netflowClusterInfoRefreshInterval = command.Flags().Duration("netflow-cluster-network-ranges-refresh-interval", 5*time.Minute, "Cluster network ranges refresh interval (0 to disable periodic refresh)")

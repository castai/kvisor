...
func NewRunCommand(version string) *cobra.Command {
	...
	var (
		...
		ephemeralStorageStatsSource = config.EphemeralStorageSourceKubelet
	...
	command.Flags().Var(&ephemeralStorageStatsSource, "ephemeral-storage-source", "Ephemeral storage metrics collection source (none, kubelet, storage-optimization, auto)")
	...
	command.Run = func(cmd *cobra.Command, args []string) {
		...
		if err := app.New(&config.Config{
			...
			Stats: config.StatsConfig{
				...
				EphemeralStorageSource: ephemeralStorageStatsSource,
			},

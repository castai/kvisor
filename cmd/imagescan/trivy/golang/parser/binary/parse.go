// Adapted from https://github.com/aquasecurity/trivy (pkg/dependency/parser/golang/binary).
...
func (p *Parser) Parse(r xio.ReadSeekerAt) ([]ftypes.Package, []ftypes.Dependency, error) {
	...
	if info.Main.Path != "" {
		...
		if len(ldflags) > 0 {
			parsedVersion = p.ParseLDFlags(info.Main.Path, ldflags)
		} else {
			r.Seek(io.SeekStart, 0)
			parsedVersion = p.extractVersionFromContents(r)
		}

		if version == "" || (strings.HasPrefix(version, "v0.0.0") && parsedVersion != "") {
...
func (p *Parser) extractVersionFromContents(reader xio.ReadSeekerAt) string {
	matchMetadata, err := MatchNamedCaptureGroupsFromReader(semverPattern, reader)
	if err != nil {
		p.logger.Info("unable to extract version from go binary reader", slog.Any("error", err))
		return ""
	}

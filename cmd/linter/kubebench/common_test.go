...
type JsonOutputFormat struct {
	Controls     []*check2.Controls `json:"Controls"`
	...
	testCases := []TestCase{
		{
			...
			Group:      &check2.Group{},
			Check:      &check2.Check{Scored: true},
			Expected:   true,
		},
		{
			...
			FilterOpts: FilterOpts{Scored: true, Unscored: false},
			Group:      &check2.Group{},
			Check:      &check2.Check{Scored: false},
			Expected:   false,
		},
...
func TestIsMaster(t *testing.T) {
	testCases := []struct {
		name            string
		cfgFile         string
		getBinariesFunc func(*viper.Viper, check2.NodeType) (map[string]string, error)
		isMaster        bool
	}{
		{
			name:    "valid config, is master and all components are running",
			cfgFile: "./kubebench-rules/config.yaml",
			getBinariesFunc: func(viper *viper.Viper, nt check2.NodeType) (strings map[string]string, i error) {
...
func TestWriteResultNoTotalsToJsonFile(t *testing.T) {
	...
	var expect []*check2.Controls
	var result []*check2.Controls
...
- id: G2
  ...
  - id: G2/C1
    ...
    scored: true
`)
	controls, err := check2.NewControls(check2.MASTER, input, "")
...
func parseControlsJsonFile(filepath string) ([]*check2.Controls, error) {
	var result []*check2.Controls
...
func parseResultNoTotalsJsonFile(filepath string) ([]*check2.Controls, error) {
	var result []*check2.Controls
...
func loadConfigForTest() (*viper.Viper, error) {
	...
	viperWithData.SetConfigFile("./kubebench-rules/config.yaml")
